...
export default function Lecturers() {
  ...
  const getTranslatedFunction = (lecturer) => {
    return t(`apiTranslations.lecturerFunctions.${lecturer.funktion}`, {
      ns: 'api-translations',
    })
  }

  const getTranslatedOrganization = (lecturer) => {
    return t(`apiTranslations.lecturerOrganizations.${lecturer.organisation}`, {
      ns: 'api-translations',
    })
  }
  ...
  return (
    <AppContainer>
      ...
      <AppBody>
        <Modal
          size="lg"
          show={!!focusedLecturer}
          onHide={() => setFocusedLecturer(null)}
        >
          {focusedLecturer && (
            <>
              <Modal.Header closeButton>
                <Modal.Title>
                  {focusedLecturer.titel} {focusedLecturer.vorname}{' '}
                  {focusedLecturer.name}
                </Modal.Title>
              </Modal.Header>
              <Modal.Body>
                <strong>{t('lecturers.modals.details.title')}</strong>:{' '}
                {focusedLecturer.titel}
                <br />
                <strong>{t('lecturers.modals.details.surname')}</strong>:{' '}
                {focusedLecturer.name}
                <br />
                <strong>{t('lecturers.modals.details.forename')}</strong>:{' '}
                {focusedLecturer.vorname}
                <br />
                <strong>
                  {t('lecturers.modals.details.organization')}
                </strong>: {getTranslatedOrganization(focusedLecturer)}
                <br />
                <strong>{t('lecturers.modals.details.function')}</strong>:{' '}
                {getTranslatedFunction(focusedLecturer)}
                <br />
                ...
                {focusedLecturer.room_short
                  ? ''
                  : focusedLecturer.raum ||
                    t('lecturers.modals.details.notAvailable')}
                <br />
                ...
                {focusedLecturer.tel_dienst && (
                  <a
                    href={`tel:${
                      focusedLecturer.tel_dienst.split(/\s(?!\d)/)[0]
                    }`}
                  >
                    {focusedLecturer.tel_dienst}
                  </a>
                ...
                <br />
                <strong>
                  {t('lecturers.modals.details.officeHours')}
                </strong>: {focusedLecturer.sprechstunde}
                <br />
                <strong>{t('lecturers.modals.details.insights')}</strong>:{' '}
                {focusedLecturer.einsichtnahme}
                <br />
              </Modal.Body>
              <Modal.Footer>
                <Button
                  variant="secondary"
                  onClick={() => setFocusedLecturer(null)}
                >
        ...
        <h3>
          {search
            ? t('lecturers.search.searchResults')
            : t('lecturers.search.personalLecturers')}
        </h3>
        <ReactPlaceholder
          type="text"
          rows={20}
          ready={filteredLecturers}
        >
          <ListGroup>
            {filteredLecturers &&
              filteredLecturers.map((x, i) => (
                <ListGroup.Item
                  key={i}
                  className={styles.item}
                  onClick={() => setFocusedLecturer(x)}
                  action
                >
