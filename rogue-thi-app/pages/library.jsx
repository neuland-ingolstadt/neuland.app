...
export default function Library() {
  ...
  function getAvailableRooms() {
    return Object.entries(reservationTime.resources)
      .map(([roomId, room], idx) => [roomId, room, idx])
      .filter(([, room]) => room.num_seats > 0)
  }
  ...
  return (
    <AppContainer>
      ...
      <AppBody>
        <Modal
          show={!!reservationDay && !!reservationTime}
          onHide={hideReservationModal}
          onShow={() => setReservationRoom(getFirstAvailableRoom())}
        >
          <Modal.Header closeButton>
            <Modal.Title>{t('library.modal.title')}</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            {t('library.modal.details.day')}:{' '}
            {reservationDay && formatFriendlyDate(reservationDay.date)}
            <br />
            {t('library.modal.details.start')}:{' '}
            {reservationTime && formatFriendlyTime(reservationTime.from)}
            <br />
            {t('library.modal.details.end')}:{' '}
            {reservationTime && formatFriendlyTime(reservationTime.to)}
            <br />
            <br />
            <Form.Group>
              ...
              >
                {reservationTime &&
                  getAvailableRooms().map(([roomId, room, idx]) => (
                    <option
                      key={idx}
                      value={roomId}
                    >
            ...
            <Form.Group>
              ...
              <Form.Control
                as="select"
                onChange={(event) => setReservationSeat(event.target.value)}
              >
                <option value={-1}>
                  {t('library.modal.seatSelection.any')}
                </option>
                {reservationTime &&
                  reservationRoom &&
                  Object.values(
                    reservationTime.resources[reservationRoom].seats
                  ).map((x, idx) => (
                    <option
                      key={idx}
                      value={x}
        ...
        <h4 className={styles.heading}>{t('library.yourReservations')}</h4>
        <ReactPlaceholder
          type="text"
          rows={3}
          ready={reservations}
            ...
            {reservations &&
              reservations.map((x, i) => (
                <ListGroup.Item key={i}>
                  ...
                  />
                  <br />
                  {formatNearDate(x.start)}: {formatFriendlyTime(x.start)} -{' '}
                  {formatFriendlyTime(x.end)}
                </ListGroup.Item>
              ))}
          </ListGroup>
        </ReactPlaceholder>

        <h4 className={styles.heading}>{t('library.availableSeats')}</h4>
        <ReactPlaceholder
          type="text"
          rows={20}
          ready={available}
              ...
              available.map((day, i) =>
                day.resource.map((time, j) => (
                  <ListGroup.Item key={i + '-' + j}>
                    {Object.values(time.resources).reduce(
                    ...
                    <div className="text-muted">
                      {t('library.details.seatsAvailable', {
                        available: Object.values(time.resources).reduce(
                          (acc, room) => acc + room.num_seats,
                          0
                        ),
                        total: Object.values(time.resources).reduce(
...
export const getStaticProps = async ({ locale }) => ({
  props: {
    ...(await serverSideTranslations(locale ?? 'en', ['library', 'common'])),
  },
})
